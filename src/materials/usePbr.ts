 ...
 */
export function usePbr(name: MaterialName, repeat?: [number, number], overrides?: Partial<{ color: string; roughness: number; metalness: number }>) {
  const m = materials[name];
  const maps = useTexture([m.albedo, m.normal, m.roughness]) as Texture[];

  return useMemo(() => {
    const [albedo, normal, roughness] = maps;
