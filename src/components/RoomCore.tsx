...
function CoreGeometry({ room }: { room: RoomDefinition }) {
  switch (room.shape) {
    case 'torus':
      return <torusKnotGeometry args={[0.98, 0.25, 72, 10]} />;
    case 'octahedron':
      return <octahedronGeometry args={[1.42, 1]} />;
    case 'box':
      return <boxGeometry args={[1.7, 1.7, 1.7, 2, 2, 2]} />;
    case 'cylinder':
      return <cylinderGeometry args={[1.0, 1.34, 2.25, 14, 2]} />;
    case 'icosahedron':
      return <icosahedronGeometry args={[1.4, 1]} />;
    default:
      return <sphereGeometry args={[1.28, 24, 16]} />;
  }
}
...
export function RoomCore({ room, active }: { room: RoomDefinition; active: boolean }) {
  ...
  useFrame(({ clock }, delta) => {
    ...
    if (coreRef.current) {
      ...
      coreRef.current.position.y = 0.65 + Math.sin(time * 0.76) * 0.14;
    }
    if (shellRef.current) {
      shellRef.current.rotation.y -= delta * 0.16;
      shellRef.current.rotation.z = time * 0.025;
      const scale = 1.32 + Math.sin(time * 0.58) * 0.035;
      shellRef.current.scale.setScalar(scale);
    }
    if (orbitRef.current) {
      orbitRef.current.rotation.y += delta * 0.19;
  ...
  return (
    <group position={[0, 0.12, 0]}>
      ...
      <group ref={orbitRef} position={[0, 0.66, 0]} rotation={[0.5, 0.2, 0.2]}>
        <mesh>
          <torusGeometry args={[2.16, 0.025, 5, 64]} />
        ...
        {[0, 1, 2].map((index) => {
          ...
          return (
            <mesh key={index} position={[Math.cos(angle) * 2.16, Math.sin(angle) * 2.16, 0]}>
              <sphereGeometry args={[0.085 + index * 0.018, 8, 6]} />
