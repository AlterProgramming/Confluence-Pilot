...
function rotatedFootprint(asset: AssetCatalogItem, scale: Vector3Tuple, yaw: number) {
  const width = Math.abs(Math.cos(yaw)) * asset.footprint[0] * scale[0]
    + Math.abs(Math.sin(yaw)) * asset.footprint[2] * scale[2];
  const depth = Math.abs(Math.sin(yaw)) * asset.footprint[0] * scale[0]
...
export function constrainAssetTransform(
  ...
  const rotated = rotatedFootprint(asset, scale, yaw);
  const fitRatio = Math.min(1, roomWidth / Math.max(rotated.width, EPSILON), roomDepth / Math.max(rotated.depth, EPSILON));
  if (fitRatio < 1) {
    scale[0] *= fitRatio;
    scale[2] *= fitRatio;
  }

  const fitted = rotatedFootprint(asset, scale, yaw);
  const height = asset.footprint[1] * scale[1];
  const halfWidth = fitted.width / 2;
  const halfDepth = fitted.depth / 2;
  ...
  return { transform, clamped, footprint: [fitted.width, height, fitted.depth] };
}

export function constrainAssetToSurface(
  proposed: AssetTransform,
  asset: AssetCatalogItem,
  ...
  const surfaceWidth = Math.max(EPSILON, surface.size[0]);
  const surfaceDepth = Math.max(EPSILON, surface.size[1]);
  const scale: Vector3Tuple = [
    clamp(Math.abs(proposed.scale[0]), 0.05, surfaceWidth / Math.max(asset.footprint[0], EPSILON)),
    clamp(Math.abs(proposed.scale[1]), 0.05, 20),
    clamp(Math.abs(proposed.scale[2]), 0.05, surfaceDepth / Math.max(asset.footprint[2], EPSILON)),
  ];
  const yaw = proposed.rotation[1];
  const rotated = rotatedFootprint(asset, scale, yaw);
  const fitRatio = Math.min(1, surfaceWidth / Math.max(rotated.width, EPSILON), surfaceDepth / Math.max(rotated.depth, EPSILON));
  if (fitRatio < 1) {
    scale[0] *= fitRatio;
    scale[2] *= fitRatio;
  }
  ...
  const halfWidth = fitted.width / 2;
  const halfDepth = fitted.depth / 2;
  const minX = surface.position[0] - surfaceWidth / 2 + halfWidth;
  const maxX = surface.position[0] + surfaceWidth / 2 - halfWidth;
  const minZ = surface.position[2] - surfaceDepth / 2 + halfDepth;
  const maxZ = surface.position[2] + surfaceDepth / 2 - halfDepth;
  const position: Vector3Tuple = [
    clamp(proposed.position[0], Math.min(minX, maxX), Math.max(minX, maxX)),
    surface.position[1],
    clamp(proposed.position[2], Math.min(minZ, maxZ), Math.max(minZ, maxZ)),
...
export function constrainPlacedAssetTransform(
  instance: PlacedAsset,
  proposed: AssetTransform,
  bounds: SceneBounds | null,
  asset: AssetCatalogItem,
  parentAsset?: AssetCatalogItem,
): BoundaryResult {
  if (instance.parentId && parentAsset) {
    ...
    if (surface) return constrainAssetToSurface(proposed, asset, surface);
  }
  return constrainAssetTransform(instance, proposed, bounds, asset);
}
...
export function transformWithinBounds(
  ...
  const result = constrainPlacedAssetTransform(instance, instance.transform, bounds, asset, parentAsset);
